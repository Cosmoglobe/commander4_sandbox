...
NSIDE   = 4096#256 #256 #4096
LMAX = 3*NSIDE-1
NTOD = 2**16
NSCAN = 27648 #27648#108 #27648#6912 #1728 #432
VERBOSE = False
...
class Gibbs:
    ...
    def read_tod_from_file(self, h5_filename, bands):
        ...
        print(f" > Rank {self.rank} > Read from file in {time.time()-t0:.2f}s.")
    ...
    def compsep_compute_Ax(self, LHS, RHS):
        ...
        while CG_solver.err > err_tol:
            for irank in range(1, self.nprocs):
                self.comm.send(CONTINUE, irank, tag=CG_TAG)
    ...
    def tod_mapmaker_purepython(self):
        ...
        self.map_rms[:] = 0.0
        for iscan in range(self.nscan):
            self.map_sky += np.bincount(self.pix[iscan], weights=self.tod[iscan]/self.sigma0_est[iscan]**2, minlength=self.npix)
            self.map_inv_var += np.bincount(self.pix[iscan], weights=1.0/self.sigma0_est[iscan]**2*np.ones(self.ntod), minlength=self.npix)
    ...
    def tod_mapmaker(self):
        ...
        maplib.mapmaker(self.map_sky, self.map_inv_var, self.tod, self.pix, self.sigma0_est, self.ntod, self.nscan, self.npix)
        self.map_rms = 1.0/np.sqrt(self.map_inv_var)
        ipix_mask = hp.query_disc(self.nside, (10,0,0), np.radians(60))  # Quick way of simulating a "mask", aka a region of infinite rms.
        self.map_rms[ipix_mask] = np.inf
    ...
    def solve(self, niter):
        for iter in range(1, niter+1):
            ...
            iband = self.rank
            np.save(f"output_MPI/sigma0_est_band_{iband:02}_c{iter:06}.npy", self.sigma0_est)
            ...
            # Write maps to file.
            hp.write_map(f'output_MPI/map_band_{iband:02}_c{iter:06}.fits', self.map_sky,
                    overwrite=True, dtype=np.float64)
            hp.write_map(f'output_MPI/rms_band_{iband:02}_c{iter:06}.fits', self.map_rms,
                    overwrite=True, dtype=np.float64)
            ...
            if self.rank == 0:
                compsep_LHS = LinearOperator(shape=((self.alm_len, self.alm_len)), matvec=self.master_LHS_func, dtype=np.complex128)
                ...
                self.alm_signal_mean = np.zeros(self.alm_len, dtype=np.complex128)
                self.map_signal_mean = np.zeros(self.npix)
                while self.comm.recv(source=0, tag=CG_TAG) == CONTINUE:
                    self.worker_LHS_func()
            if self.rank == 0:
                ...
                self.alm_signal_fluct = np.zeros(self.alm_len, dtype=np.complex128)
                self.map_signal_fluct = np.zeros(self.npix)
                while self.comm.recv(source=0, tag=CG_TAG) == CONTINUE:
                    self.worker_LHS_func()
            self.comm.Barrier()
            self.comm.Bcast(self.map_signal_mean, root=0)
            self.comm.Bcast(self.map_signal_fluct, root=0)
            if self.rank == 0:
            ...
            if self.rank == 0:
                ...
                hp.write_map(f'output_MPI/mapx_c{iter:06}.fits', self.map_signal_mean,
                        overwrite=True, dtype=np.float64)
...
if __name__ == "__main__":
    ...
    stats.print_stats(5)
    stats.dump_stats(f'stats-{gibbs.rank}')