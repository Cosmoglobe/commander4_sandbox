...
class Gibbs:
    def __init__(self):
        ...
        np.random.seed(128 + self.rank*15331)
    ...
    def LHS_func(self, x):
        ...
        self.comm.Allreduce([ATYTNYAx, MPI.DOUBLE],
                         [LHS_sum, MPI.DOUBLE],
                         op=MPI.SUM)
    ...
    def compsep_compute_Ax(self, LHS, RHS):
        ...
        while CG_solver.err > err_tol:
            CG_solver.step()
            iter += 1
            if VERBOSE and iter%10 == 1 and self.rank == 0:
                print(f"Rank {self.rank:3d} CG iter {iter:3d} - Residual {CG_solver.err:.3e}")
            if iter >= maxiter:
                if self.rank == 0:
