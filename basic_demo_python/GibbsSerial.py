...
filename = '../src/python/preproc_scripts/tod_example_256_1.0_b20.h5'
outdir = "output_256_1.0_b20"
ngibbs = 50
FWHM    = np.pi/180.0*np.ones(5)#0.16666*np.pi/180.0*np.ones(5)
NSIDE   = 256
LMAX = 3*NSIDE-1
NTOD = 2**16
NSCAN = 108 #27648 #6912 #1728 #432
VERBOSE = False
...
class Gibbs:
    ...
    def get_RHS_eqn_fluct(self):
        ...
        RHS_sum = np.zeros(self.alm_len, dtype=np.complex128)
        np.random.seed(199 + self.iter*15331)
        CYTomega0 = hp.synalm(1.0/self.Cl_sample, self.lmax)
        RHS_sum += CYTomega0

        for iband in range(self.nband):
            np.random.seed(4321 + self.iter*15331 + iband)
    ...
    def solve(self, niter):
        for iter in range(1, niter+1):
            ...
            for iband in range(self.nband):
                np.save(f"{outdir}/sigma0_est_band_{iband:02}_c{iter:06}.npy", self.sigma0_est)
            ...
            # Write maps to file.
            for iband in range(self.nband):
                hp.write_map(f'{outdir}/map_band_{iband:02}_c{iter:06}.fits', self.map_sky[iband],
                        overwrite=True, dtype=np.float64)
                hp.write_map(f'{outdir}/rms_band_{iband:02}_c{iter:06}.fits', self.map_rms[iband],
                        overwrite=True, dtype=np.float64)
            ...
            hp.write_map(f'{outdir}/mapx_c{iter:06}.fits', self.map_signal_mean,
                    overwrite=True, dtype=np.float64)
            hp.write_map(f'{outdir}/mapy_c{iter:06}.fits', self.map_signal_fluct,
                    overwrite=True, dtype=np.float64)
            np.save(f"{outdir}/Cl_sample_c{iter:06}.npy", self.Cl_sample)
...
if __name__ == "__main__":
    ...
    np.random.seed(128)
    gibbs = Gibbs()
    gibbs.read_tod_from_file(filename, ['030', '070', '100', '217', '353'])
    gibbs.solve(ngibbs)
