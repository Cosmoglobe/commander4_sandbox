...
map_hits = hp.read_map(f"output/hits_band_{iband:02}_c000001.fits")
...
for iter in tqdm.tqdm(range(1, 1000)):
    if os.path.exists(f"plots/Cl_iter{iter:06}.png"):
        continue
    mapx = hp.read_map(f"output/mapx_band_{iband:02}_c{iter:06}.fits") 
    hp.mollview(mapx, title=f"mean field map, iter {iter}", min=-250, max=250, cmap="RdBu_r")
    plt.savefig(f"plots/mapx_iter{iter:06}.png")
    mapy = hp.read_map(f"output/mapy_band_{iband:02}_c{iter:06}.fits")
    ...
    Cl_sample = np.load(f"output/Cl_sample_band_{iband:02}_c{iter:06}.npy")
    ...
    plt.figure()
    Z = ell*(ell+1)/(2*np.pi)
    plt.semilogx(ell[2:], Z[2:]*Clx[2:], label="Mean field map Cl")
    plt.semilogx(ell[2:], Z[2:]*Clxy[2:], label="Mean field + fluct Cl")
    plt.semilogx(ell[2:], Z[2:]*Cl_sample[2:], label="Gibbs sampled Cl")
    plt.semilogx(ell[2:], Dl[2:], label="True Cl", color='k', zorder=10)
    plt.semilogx(ell[2:], Z[2:]*Cl_sky[2:], label="Observed sky Cl")
    plt.legend()
    plt.ylim(0, 7500)
